...
const db = mysql.createConnection({
    ...
    user: 'root',
    password: '',
    database: 'penyewaan_SewaAlatBersih'
});
...
const server = http.createServer((req,res) =>{
    if(req.method === 'GET') {
        ...
        req.on('end',() => {
            ...
            const {name,email,tool,prices,message} = parsed;

               const sql = 'insert into contacts (name, email, tool, prices, message) values(?, ?, ?,?)';
            db.query(sql,[name, email, tool,prices, message], (err) => {
                if(err) {
